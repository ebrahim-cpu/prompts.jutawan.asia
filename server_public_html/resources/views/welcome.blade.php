...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark">
    ...
    <body class="antialiased relative selection:bg-primary selection:text-white">
        ...
        <!-- Expiry / Expired Warning for Welcome Page -->
        @auth
            @if(auth()->user()->isSubscriptionExpiringSoon())
                @php $isExpired = auth()->user()->isSubscriptionExpired(); @endphp
                <!-- Countdown & Expiry Logic for Welcome Page -->
                <div x-data="{ 
                    ...
                    <div class="hidden sm:block pt-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto" x-show="showWarning" x-transition>
                        <div class="bg-gradient-to-r from-orange-500/10 to-yellow-500/10 border border-orange-500/30 rounded-2xl p-4 shadow-lg shadow-orange-500/5 flex items-center justify-between gap-4">
                            <div class="flex items-center gap-3">
                                ...
                                <template x-if="isExpired">
                                    <div class="w-10 h-10 rounded-xl bg-red-500/20 flex items-center justify-center border border-red-500/30 shrink-0 text-xl">
                                        🚨
                                    </div>
                                </template>
                                <div>
                                    <h3 class="text-sm font-bold text-white mb-0.5" x-text="isExpired ? '🚨 Langganan Telah Tamat' : '⚠️ Langganan Hampir Tamat'"></h3>
                                    <p class="text-xs text-orange-200/80" x-text="isExpired ? 'Langganan Premium anda telah tamat. Segera perbaharui untuk kekal mendapat akses.' : 'Premium akan ditamatkan tidak lama lagi. Segera perbaharui.'"></p>
                                </div>
                            </div>
                            ...
                            <div x-show="open" x-cloak class="fixed inset-0 z-[100]" @click.self="open = false">
                                ...
                                <div x-show="open" 
                                    ...
                                    <div class="flex flex-col items-center text-center">
                                        <h3 class="text-xl font-extrabold text-white mb-2 text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-yellow-400" x-text="isExpired ? 'Langganan Telah Tamat!' : 'Langganan Hampir Tamat!'"></h3>
                                        <p class="text-sm text-gray-300 mb-4" x-text="isExpired ? 'Akses Premium anda telah tamat. Perbaharui sekarang!' : 'Akses Premium anda akan tamat dalam masa:'"></p>
        ...
        <div class="pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto text-center">
            ...
            <div class="flex flex-wrap justify-center gap-4 sm:gap-6 mb-12 animate-fade-in-up" style="animation-delay: 0.2s;">
                ...
                <div class="glass-card rounded-2xl px-6 py-4 text-center min-w-[120px]">
                    <div class="text-3xl font-extrabold text-cyan-400">{{ number_format($upcomingPrompts ?? 0) }}</div>
                    <div class="text-xs text-gray-400 mt-1 uppercase tracking-wider font-medium">Upcoming</div>
                </div>
                ...
                <div class="glass-card rounded-2xl p-2.5" style="overflow: visible;">
                    ...
                    <div class="flex flex-wrap items-center gap-2 mt-2.5 pt-2.5 border-t border-white/5">
                        ...
                        <div class="flex gap-1 flex-shrink-0">
                            ...
                            <a href="{{ route('home', array_merge(request()->except('page'), ['filter' => 'upcoming'])) }}" class="filter-link px-3 py-1.5 rounded-lg text-xs font-medium transition {{ request('filter') === 'upcoming' ? 'bg-cyan-500/20 text-cyan-400' : 'text-gray-500 hover:text-cyan-400 hover:bg-cyan-500/10' }}">⏳ Upcoming</a>
                        </div>
        ...
        <div id="prompt-results" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                @foreach ($prompts as $index => $prompt)
                    ...
                    <div class="prompt-card"
                        ...
                        <!-- Card Container -->
                        <div class="flip-container rounded-2xl shadow-lg shadow-black/30 hover:shadow-2xl hover:shadow-pink-500/20 transition-shadow duration-300"
                             :class="{ 'flipped': flipped }">

                            <div class="flip-card">
                                
                                <!-- ===== FRONT FACE ===== -->
                                <div class="flip-front bg-slate-800 border border-white/10 group">
                                    <!-- Photo Click Link (Opens dedicated Photo & Related Prompts page) -->
                                    <a href="{{ route('prompts.show', $prompt->id) }}" class="absolute inset-0 z-20 block focus:outline-none" title="Klik untuk lihat foto & prompt berkaitan"></a>

                                    <!-- Full portrait image / Slideshow -->
                                    @php $images = $prompt->images ?? []; $imgCount = count($images); @endphp
                                    @if($imgCount > 0)
                                        @if($imgCount == 1)
                                            <img src="{{ $images[0] }}" alt="{{ $prompt->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 {{ (!$hasAccess && !$isUpcoming) ? 'brightness-50' : '' }}">
                                        @else
                                            <div class="w-full h-full relative" x-data="{ currentSlide: 0, total: {{ $imgCount }}, timer: null }" 
                                                ...
                                                @foreach($images as $imgIdx => $img)
                                                    <img src="{{ $img }}" alt="{{ $prompt->title }}" 
                                                         class="absolute inset-0 w-full h-full object-cover transition-opacity duration-1000 ease-in-out group-hover:scale-105 transition-transform duration-500 {{ (!$hasAccess && !$isUpcoming) ? 'brightness-50' : '' }}"
                                    ...
                                    <div class="absolute inset-x-0 bottom-0 p-4 z-40">
                                        <h3 class="text-base font-bold text-white leading-tight mb-1 drop-shadow-lg group-hover:text-pink-300 transition-colors">{{ $prompt->title }}</h3>
                                    ...
                                    <!-- Tier Badge -->
                                    @if($isUpcoming)
                                        <div class="absolute top-3 right-3 z-30 bg-cyan-500/90 backdrop-blur-sm text-white text-[10px] font-bold px-2.5 py-1 rounded-full shadow-lg flex items-center gap-1">
                                            ⏳ Upcoming
                                        </div>
                                    @elseif($isPremium)
                                        <div class="absolute top-3 right-3 z-30 bg-gradient-to-r from-yellow-400 to-yellow-600 text-black text-[10px] font-bold px-2.5 py-1 rounded-full shadow-lg flex items-center gap-1">
                                            <svg class="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                            Premium
                                        </div>
                                    @else
                                        <div class="absolute top-3 right-3 z-30 bg-green-500/80 backdrop-blur-sm text-white text-[10px] font-bold px-2.5 py-1 rounded-full">
                                            Free
                                        </div>
                                    @endif

                                    <!-- Quick Flip Toggle Button -->
                                    <button type="button" @click.stop.prevent="flipped = !flipped" 
                                            class="absolute top-3 left-3 z-30 bg-black/60 hover:bg-pink-600 backdrop-blur-sm text-white text-[10px] font-medium px-2.5 py-1 rounded-full flex items-center gap-1 transition-colors border border-white/10"
                                            title="Pratonton teks prompt">
                                    ...
                                    <div class="w-full h-full flex flex-col p-5">
                                        ...
                                        <div class="mb-3 shrink-0">
                                            <div class="flex items-center justify-between mb-2">
                                                ...
                                                @if($isUpcoming)
                                                    <span class="shrink-0 bg-cyan-500/20 text-cyan-400 text-[10px] font-bold px-2 py-0.5 rounded-full border border-cyan-500/30">⏳ Upcoming</span>
                                                @elseif($isPremium)
                                        ...
                                        <div class="flex-grow overflow-y-auto min-h-0 mb-4 pr-1" style="scrollbar-width: thin; scrollbar-color: #475569 transparent;">
                                            @if($isUpcoming)
                                                <div class="h-full flex flex-col items-center justify-center text-center px-4 py-6">
                                                    <div class="w-14 h-14 rounded-2xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center mb-3">
                                                        <span class="text-2xl">⏳</span>
                                                    </div>
                                                    <p class="text-sm font-bold text-white mb-1">Akan Datang / Upcoming</p>
                                                    <p class="text-xs text-gray-400 leading-relaxed">Prompt ini masih dalam peringkat draf / belum diterbitkan. Nantikan kemas kini seterusnya!</p>
                                                </div>
                                            @elseif(!$hasAccess)
                                        ...
                                        <div class="shrink-0 relative text-center">
                                            <span class="inline-block w-full py-2 rounded-xl text-xs font-bold text-cyan-400 bg-cyan-500/10 border border-cyan-500/20">
            ...
            <!-- Cool Glassmorphic Pagination -->
            @if($prompts->hasPages())
            <div class="mt-12 flex flex-col items-center gap-4">
                
                <!-- Results Count Summary -->
                <div class="text-xs text-gray-400 flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-1.5 rounded-full shadow-inner">
                    <span class="relative flex h-2 w-2">
                        <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-pink-400 opacity-75"></span>
                        <span class="relative inline-flex rounded-full h-2 w-2 bg-pink-500"></span>
                    </span>
                    Menunjukkan <strong class="text-white font-mono">{{ $prompts->firstItem() }}</strong> – <strong class="text-white font-mono">{{ $prompts->lastItem() }}</strong> daripada <strong class="text-white font-mono">{{ $prompts->total() }}</strong> prompt
                </div>

                <!-- Cool Floating Pagination Controls -->
                <div class="glass-card rounded-2xl p-2.5 inline-flex items-center gap-1.5 shadow-2xl shadow-purple-950/40 border border-white/10 backdrop-blur-2xl flex-wrap justify-center">
                    
                    {{-- First Page Link --}}
                    @if ($prompts->onFirstPage())
                        <span class="px-3 py-2 rounded-xl text-xs font-semibold text-gray-600 bg-white/5 cursor-not-allowed flex items-center gap-1" title="Halaman Pertama">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7"/></svg>
                        </span>
                    @else
                        <a href="{{ $prompts->url(1) }}" class="filter-link px-3 py-2 rounded-xl text-xs font-semibold text-gray-300 hover:text-white bg-white/5 hover:bg-gradient-to-r hover:from-purple-600 hover:to-pink-600 transition-all duration-200 flex items-center gap-1 hover:shadow-lg hover:shadow-purple-500/30 active:scale-95" title="Halaman Pertama">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7"/></svg>
                        </a>
                    @endif
                    ...
                    {{-- Previous Page Link --}}
                    @if ($prompts->onFirstPage())
                        <span class="px-3.5 py-2 rounded-xl text-xs font-semibold text-gray-600 bg-white/5 cursor-not-allowed flex items-center gap-1">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                            <span class="hidden sm:inline">Sebelum</span>
                        </span>
                    @else
                        <a href="{{ $prompts->previousPageUrl() }}" class="filter-link px-3.5 py-2 rounded-xl text-xs font-semibold text-gray-300 hover:text-white bg-white/5 hover:bg-gradient-to-r hover:from-purple-600 hover:to-pink-600 transition-all duration-200 flex items-center gap-1 hover:shadow-lg hover:shadow-purple-500/30 active:scale-95">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                            <span class="hidden sm:inline">Sebelum</span>
                        </a>
                    @endif
                    ...
                    @php
                        ...
                        $lastPage = $prompts->lastPage();
                        $start = max(1, $currentPage - 2);
                        $end = min($lastPage, $currentPage + 2);
                    @endphp

                    @if($start > 1)
                        <a href="{{ $prompts->url(1) }}" class="filter-link w-9 h-9 rounded-xl text-xs font-bold flex items-center justify-center text-gray-300 hover:text-white bg-white/5 hover:bg-white/15 transition-all duration-200 hover:scale-105">1</a>
                        @if($start > 2)
                            <span class="w-7 h-9 flex items-center justify-center text-gray-500 text-xs select-none">...</span>
                        @endif
                    @endif

                    @for ($page = $start; $page <= $end; $page++)
                        @if ($page == $currentPage)
                            <span class="w-9 h-9 rounded-xl text-xs font-extrabold flex items-center justify-center text-white bg-gradient-to-r from-purple-600 to-pink-600 shadow-lg shadow-purple-500/40 scale-105 ring-2 ring-pink-400/40 select-none">
                                {{ $page }}
                            </span>
                        @else
                            <a href="{{ $prompts->url($page) }}" class="filter-link w-9 h-9 rounded-xl text-xs font-bold flex items-center justify-center text-gray-300 hover:text-white bg-white/5 hover:bg-white/15 border border-white/5 hover:border-pink-500/30 transition-all duration-200 hover:scale-105 active:scale-95">
                                {{ $page }}
                            </a>
                        @endif
                    @endfor
                    ...
                    @if($end < $lastPage)
                        @if($end < $lastPage - 1)
                            <span class="w-7 h-9 flex items-center justify-center text-gray-500 text-xs select-none">...</span>
                        @endif
                        <a href="{{ $prompts->url($lastPage) }}" class="filter-link w-9 h-9 rounded-xl text-xs font-bold flex items-center justify-center text-gray-300 hover:text-white bg-white/5 hover:bg-white/15 transition-all duration-200 hover:scale-105">{{ $lastPage }}</a>
                    @endif
                    ...
                    @if ($prompts->hasMorePages())
                        <a href="{{ $prompts->nextPageUrl() }}" class="filter-link px-3.5 py-2 rounded-xl text-xs font-semibold text-gray-300 hover:text-white bg-white/5 hover:bg-gradient-to-r hover:from-purple-600 hover:to-pink-600 transition-all duration-200 flex items-center gap-1 hover:shadow-lg hover:shadow-purple-500/30 active:scale-95">
                    ...
                    {{-- Last Page Link --}}
                    @if ($prompts->hasMorePages())
                        <a href="{{ $prompts->url($lastPage) }}" class="filter-link px-3 py-2 rounded-xl text-xs font-semibold text-gray-300 hover:text-white bg-white/5 hover:bg-gradient-to-r hover:from-purple-600 hover:to-pink-600 transition-all duration-200 flex items-center gap-1 hover:shadow-lg hover:shadow-purple-500/30 active:scale-95" title="Halaman Akhir">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 5l7 7-7 7M5 5l7 7-7 7"/></svg>
                        </a>
                    @else
                        <span class="px-3 py-2 rounded-xl text-xs font-semibold text-gray-600 bg-white/5 cursor-not-allowed flex items-center gap-1" title="Halaman Akhir">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 5l7 7-7 7M5 5l7 7-7 7"/></svg>
                        </span>
                    @endif
                ...
                @if($lastPage > 3)
                    <div x-data="{ targetPage: '' }" class="inline-flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-1.5 text-xs text-gray-400 shadow-sm">
                        <span>Lompat ke:</span>
                        <input type="number" min="1" max="{{ $lastPage }}" x-model="targetPage" placeholder="Halaman #"
                               @keyup.enter="if(targetPage >= 1 && targetPage <= {{ $lastPage }}) { let url = new URL(window.location.href); url.searchParams.set('page', targetPage); ajaxFilter(url.toString()); }"
                               class="w-16 bg-gray-900/80 border border-white/10 rounded-lg px-2 py-1 text-xs text-white text-center focus:ring-1 focus:ring-pink-500 focus:outline-none">
                        <button type="button" 
                                @click="if(targetPage >= 1 && targetPage <= {{ $lastPage }}) { let url = new URL(window.location.href); url.searchParams.set('page', targetPage); ajaxFilter(url.toString()); }"
                                class="px-2.5 py-1 rounded-lg bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold transition hover:opacity-90 shadow-md shadow-pink-500/20 text-xs">
                            Pergi →
                        </button>
                    </div>
                @endif
        ...
        <script>
            ...
            async function ajaxFilter(url) {
                ...
                try {
                    ...
                    // Smooth scroll up to results top if pagination was clicked
                    if (url.includes('page=')) {
                        const target = document.getElementById('prompt-results');
                        if (target) {
