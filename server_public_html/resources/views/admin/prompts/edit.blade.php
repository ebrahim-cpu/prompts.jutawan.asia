            ...
            <div class="bg-gray-800/50 backdrop-blur-sm rounded-2xl border border-white/10 overflow-hidden shadow-2xl">
                <form action="{{ route('admin.prompts.update', $prompt->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-8 space-y-6">
                        ...
                        <!-- Status Checkboxes (Upcoming, Premium, Featured) -->
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <!-- Is Upcoming -->
                            <div class="bg-cyan-500/10 border border-cyan-500/30 rounded-xl p-4">
                                <label for="is_upcoming" class="flex items-center gap-3 cursor-pointer">
                                    <input id="is_upcoming" name="is_upcoming" type="checkbox" value="1" {{ old('is_upcoming', $prompt->is_upcoming) ? 'checked' : '' }}
                                        class="h-5 w-5 rounded text-cyan-500 border-cyan-600 bg-gray-900 focus:ring-cyan-500 transition">
                                    <div>
                                        <span class="font-bold text-cyan-300 text-sm">⏳ Tandakan sebagai Upcoming (Draf)</span>
                                        <p class="text-xs text-gray-400 mt-0.5">Prompt belum bersedia diterbitkan. Bukan free atau premium, tetapi boleh di-Featured.</p>
                                    </div>
                                </label>
                            </div>
                            ...
                            <div class="bg-pink-500/10 border border-pink-500/30 rounded-xl p-4">
                                <label for="is_featured" class="flex items-center gap-3 cursor-pointer">
                                    <input id="is_featured" name="is_featured" type="checkbox" value="1" {{ old('is_featured', $prompt->is_featured) ? 'checked' : '' }}
