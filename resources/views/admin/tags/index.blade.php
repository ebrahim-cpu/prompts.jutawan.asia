<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div>
                <h2 class="text-2xl font-bold text-white">Pengurusan Tag</h2>
                <p class="text-sm text-gray-400 mt-1">Tambah, kemaskini dan padam tag prompt.</p>
            </div>
    ...
    <div class="py-8" x-data="{ showModal: false, editMode: false, formAction: '{{ route('admin.tags.store') }}', tagName: '', tagId: null }">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-gray-800/50 backdrop-blur-sm rounded-2xl border border-white/10 p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    <p class="text-lg font-bold text-white">Jumlah Tag: {{ $totalTags }}</p>
                    <p class="text-xs text-gray-400">Tag yang dicipta di sini boleh dipilih secara berbilang (multiple select) semasa menambah/mengedit prompt.</p>
                </div>
                                ...
                                <tr class="hover:bg-white/5 transition">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <form action="{{ route('admin.tags.destroy', $t->id) }}" method="POST" onsubmit="return confirmDelete(this, 'Adakah anda pasti mahu memadam tag ini?');">
                                                @csrf
                                                @method('DELETE')
        ...
        <template x-teleport="body">
            <div x-show="showModal" x-cloak class="fixed inset-0 z-[150] flex items-center justify-center p-4">
                ...
                <div class="relative z-10 w-full max-w-md bg-gray-800 border border-white/10 rounded-2xl p-6 shadow-2xl">
                    <h3 class="text-lg font-bold text-white mb-4" x-text="editMode ? 'Kemaskini Tag' : 'Tambah Tag Baru'"></h3>
                    ...
                    <form :action="formAction" method="POST" class="space-y-4">
                        ...
                        <div>
                            <label for="name" class="block text-xs font-bold text-gray-300 mb-1">Nama Tag</label>
